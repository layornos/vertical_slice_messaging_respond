...
fun main(args: Array<String>) = mainBody("Repository") {
    ...
    configuration.apply {
        ...
        setProperty("hibernate.connection.username", arguments.database_user)
        setProperty("hibernate.connection.password", arguments.database_password)
    }
...
class CommandLineArguments(parser: ArgParser) {
    ...
    val database_user by parser.storing("--dbuser", "-u", help = "The username for connecting to the database").default {
        getEnvOrNull("DATABASE_USER") ?: "root"
    }
    val database_password by parser.storing("--dbpassword", "-p", help = "The password for connecting to the database").default {
